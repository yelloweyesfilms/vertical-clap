...
export default function BlogEnPost({ post, otherPosts }) {
  if (!post) return null;
  ...
  const articleSchema = {
    ...
    "mainEntityOfPage": `${SITE}/en/blog/${post.slug}`,
  };
  ...
  return (
    <>
      <Head>
        ...
        <link rel="canonical" href={`${SITE}/en/blog/${post.slug}`} />
        <link rel="alternate" hrefLang="fr" href={`${SITE}/blog/${post.slug}`} />
        <link rel="alternate" hrefLang="en" href={`${SITE}/en/blog/${post.slug}`} />
        <meta property="og:title" content={post.title} />
        <meta property="og:description" content={post.description} />
        <meta property="og:url" content={`${SITE}/en/blog/${post.slug}`} />
        ...
        {otherPosts.length > 0 && (
          <div style={{ borderTop: `1px solid ${BORDER}`, padding: "64px 40px" }}>
            <div style={{ maxWidth: 760, margin: "0 auto" }}>
              <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: MUTED, marginBottom: 32 }}>More articles</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {otherPosts.map(p => (
                  <a key={p.slug} href={`/en/blog/${p.slug}`} style={{ display: "flex", alignItems: "center", gap: 16, padding: "20px 24px", background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 16 }}>
...
export async function getStaticPaths() {
  return {
    paths: POSTS_EN.map(p => ({ params: { slug: p.slug } })),
    fallback: false,
  };
}

export async function getStaticProps({ params }) {
  const post = POSTS_EN.find(p => p.slug === params.slug) || null;
  const otherPosts = POSTS_EN.filter(p => p.slug !== params.slug);
