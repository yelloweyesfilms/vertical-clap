...
export default async function handler(req, res) {
  ...
  try {
    if (action === "bible") {
      const { mode, casting, univers, secret, format, duree, genre, lieu, ambiance, tropes, drama, dramaPremium, lang, castingIA, ambianceVisuelle, remakeInspiration } = payload;
      const ck = `bible:${mode}:${casting}:${univers}:${secret}:${format}:${duree}:${genre || ""}:${lieu || ""}:${ambiance || ""}:${tropes || ""}:${JSON.stringify(drama || {})}:${JSON.stringify(dramaPremium || {})}:${lang || "fr"}:${remakeInspiration || ""}`;
    ...
    if (action === "script") {
      const { ep, bible, mode, duree, prevEps, lang, ambianceVisuelle: scriptAV, budgetInstr, tropes: scriptTropes, dramaPremium: scriptDramaPremium, isChoix = false, genreFormat, choixContext } = payload;
      const md = mode === "fast"
      ...
      const scriptTropesInstr = scriptTropes ? `\nTROPES & CODES NARRATIFS: ${scriptTropes}. Intègre ces codes dans les dialogues et les situations.` : "";
      const choixPrecedentInstr = choixContext ? `\n\nCHOIX DU SPECTATEUR (épisode précédent): L'utilisateur a choisi "${choixContext.label}". Conséquence attendue: "${choixContext.consequence}". Intègre cette direction narrative de façon cohérente et visible dans cet épisode — les personnages se comportent selon ce qui a été déclenché. Ne mentionne pas explicitement le choix, montre juste les conséquences.` : "";
