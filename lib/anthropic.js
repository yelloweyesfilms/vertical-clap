...
function smartParse(text) {
  ...
  try {
    ...
    // Last resort: strip trailing incomplete values and retry
    const cleaned = json
      .replace(/,\s*"[^"]*":\s*"[^"]*$/, "")
      .replace(/,\s*"[^"]*":\s*$/, "")
      .replace(/,\s*"[^"]*$/, "")
    ...
    const final = cleaned + stack2.reverse().join("");
...
async function withRetry(fn, maxAttempts = 3) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const isOverload  = e.status === 529 || e.status === 503 || e.status === 500 || e.status === 502;
      if (attempt === maxAttempts || (!isRateLimit && !isOverload)) throw e;
      // Rate limit: wait 20s (resets per minute), overload: exponential backoff
      const delay = isRateLimit ? 20000 : attempt * 2000;
