  ...
  try {
    await resend.emails.send({
      ...
      subject: plan === "premium"
        ? "Bienvenue sur VerticalClap Premium 🎭 — ton studio est prêt"
        : "Bienvenue sur VerticalClap Créateur ⚡ — ton studio est prêt",
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:${DARK};padding:32px 16px;">
    <tr><td align="center">
      <table width="100%" style="max-width:580px;background:${CARD};border-radius:20px;overflow:hidden;border:1px solid ${BORDER};">
        ...
        <!-- HEADER LOGO -->
        <tr>
          <td style="background:${DARK};padding:20px 32px;border-bottom:1px solid ${BORDER};">
...
function welcomeHtml(plan) {
  ...
  const planName = isPremium ? "PREMIUM" : "CRÉATEUR";
  ...
  const upsellBlock = !isPremium ? `
    <table width="100%" cellpadding="0" cellspacing="0" style="background:rgba(168,85,247,0.06);border:1px solid rgba(168,85,247,0.2);border-radius:14px;margin-top:24px;">
      <tr><td style="padding:20px 24px;">
        ...
        <a href="${APP_URL}/tarifs" style="font-size:13px;font-weight:700;color:${VIO};text-decoration:none;letter-spacing:0.3px;">Passer à Premium →</a>
      </td></tr>
    </table>` : "";

  return base(`Bienvenue sur VerticalClap ${isPremium ? "Premium" : "Créateur"}`, `
