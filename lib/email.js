...
function base(title, body, lang = "fr") {
  const footer = lang === "en"
    ? `© 2026 VerticalClap · <a href="${APP_URL}/confidentialite" style="color:#475569;text-decoration:none;">Privacy</a> · <a href="${APP_URL}/cgu" style="color:#475569;text-decoration:none;">Terms</a> · <a href="${APP_URL}/contact" style="color:#475569;text-decoration:none;">Contact</a>`
    : `© 2026 VerticalClap · <a href="${APP_URL}/confidentialite" style="color:#475569;text-decoration:none;">Confidentialité</a> · <a href="${APP_URL}/cgu" style="color:#475569;text-decoration:none;">CGU</a> · <a href="${APP_URL}/contact" style="color:#475569;text-decoration:none;">Contact</a>`;
  return `<!DOCTYPE html>
<html lang="${lang}">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title></head>
<body style="margin:0;padding:0;background:#09090f;font-family:Helvetica,Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#09090f;padding:40px 16px;">
    <tr><td align="center">
      <table width="100%" style="max-width:580px;background:#0d0d1a;border-radius:20px;overflow:hidden;border:1px solid rgba(255,255,255,0.08);">
        <!-- Header -->
        <tr>
          <td style="background:#09090f;padding:24px 32px;text-align:center;border-bottom:1px solid rgba(255,255,255,0.06);">
            <table cellpadding="0" cellspacing="0" style="margin:0 auto;">
              <tr>
                <td style="border-left:3px solid ${RED};padding-left:10px;vertical-align:middle;">
                  <span style="font-family:Helvetica,Arial,sans-serif;font-size:9px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:rgba(255,255,255,0.45);display:block;line-height:1;margin-bottom:3px;">VERTICAL</span>
                  <span style="font-family:Georgia,serif;font-size:22px;font-weight:900;color:${RED};letter-spacing:-0.5px;line-height:1;">CLAP</span>
                </td>
              </tr>
            </table>
          </td>
        ...
        <!-- Gradient bar -->
        <tr><td style="height:3px;background:linear-gradient(90deg,${RED},${VIO});padding:0;"></td></tr>
        <!-- Body -->
        <tr><td style="padding:36px 32px 28px;">${body}</td></tr>
        <!-- Footer -->
        <tr>
          <td style="background:#09090f;padding:20px 32px;text-align:center;border-top:1px solid rgba(255,255,255,0.06);">
            <p style="font-size:12px;color:#475569;margin:0 0 6px;">${footer}</p>
          </td>
        </tr>
      </table>
    </td></tr>
...
function newsletterWelcomeHtml(lang = "fr") {
  ...
  const articles = isFr ? [
    ...
    { title: "TikTok vs DramaBox vs ReelShort — where to publish?", url: `${APP_URL}/en/blog/tiktok-vs-dramabox-vs-reelshort-ou-publier-micro-drama`, label: "Distribution", time: "9 min", color: "#4ade80", emoji: "🌍" },
  ];
  ...
  const t = isFr ? {
    welcome: "Bienvenue 🎬",
    hero: "Le studio IA des créateurs<br>de micro-dramas verticaux.",
    heroSub: "Chaque semaine : conseils, tendances et ressources pour créer et monétiser tes séries. Rien de superflu.",
    badge: "📱 Bible · Scripts · Hooks · Cliffhangers",
    badgeSub: "Prêts à tourner en 5 minutes",
    step1title: "1. Le Mixeur", step1desc: "Choisis ton univers, casting &amp; secret central",
    step2title: "2. La Bible", step2desc: "Titre, logline, personnages générés en live",
    step3title: "3. Tourne", step3desc: "Script + téléprompteur. Prêt à filmer.",
    platforms: "Compatible avec toutes les plateformes",
    articlesLabel: "Pour commencer — 3 articles essentiels",
    readTime: "de lecture",
    ctaTitle: "Regarde comment ça marche →",
    ctaSub: "Bible + scripts d'une série complète. Gratuit à consulter.",
    ctaBtn: "Voir les séries exemples →",
  } : {
    welcome: "Welcome 🎬",
    hero: "The AI studio for vertical<br>micro-drama creators.",
    heroSub: "Every week: tips, trends and resources to create and monetize your series. Nothing superfluous.",
    badge: "📱 Bible · Scripts · Hooks · Cliffhangers",
    badgeSub: "Ready to shoot in 5 minutes",
    step1title: "1. The Mixer", step1desc: "Choose your universe, cast &amp; central secret",
    step2title: "2. The Bible", step2desc: "Title, logline, characters generated live",
    step3title: "3. Shoot", step3desc: "Script + teleprompter. Ready to film.",
    platforms: "Compatible with all platforms",
    articlesLabel: "To get started — 3 essential articles",
    readTime: "read",
    ctaTitle: "See how it works →",
    ctaSub: "Bible + scripts of a complete series. Free to browse.",
    ctaBtn: "View example series →",
  };
  ...
  return base(isFr ? "Bienvenue dans la newsletter VerticalClap" : "Welcome to the VerticalClap newsletter", `
    ...
    <div style="margin:-36px -32px 28px;line-height:0;">
      <img src="${APP_URL}/${isFr ? 'banniere%20hero.png' : 'banniere%20EN.png'}" alt="VerticalClap" width="580" style="width:100%;max-width:580px;display:block;border-radius:0;" />
    </div>

    <!-- HERO -->
    <div style="text-align:center;margin-bottom:28px;">
      <p style="font-size:11px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:${RED};margin:0 0 16px;">${t.welcome}</p>
      <h1 style="font-family:Georgia,serif;font-size:26px;font-weight:900;color:#f1f5f9;margin:0 0 14px;letter-spacing:-0.5px;line-height:1.25;">
        ${t.hero}
      </h1>
      <p style="font-size:14px;color:#94a3b8;line-height:1.7;margin:0 0 20px;">
        ${t.heroSub}
      </p>
      <div style="display:inline-block;background:rgba(232,92,58,0.1);border:1px solid rgba(232,92,58,0.25);border-radius:10px;padding:12px 20px;">
        <p style="font-size:13px;color:#f1f5f9;margin:0;font-weight:600;">
          ${t.badge}<br>
          <span style="font-size:12px;color:#94a3b8;font-weight:400;">${t.badgeSub}</span>
        </p>
      </div>
    </div>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
      <tr>
        <td style="background:rgba(232,92,58,0.08);border:1px solid rgba(232,92,58,0.2);border-radius:12px;padding:18px;vertical-align:top;width:32%;">
          <p style="font-size:26px;margin:0 0 8px;text-align:center;">🎲</p>
          <p style="font-size:12px;font-weight:800;color:#f1f5f9;margin:0 0 6px;text-align:center;">${t.step1title}</p>
          <p style="font-size:11px;color:#94a3b8;margin:0;text-align:center;line-height:1.5;">${t.step1desc}</p>
        </td>
        <td style="width:2%;"></td>
        <td style="background:rgba(168,85,247,0.08);border:1px solid rgba(168,85,247,0.2);border-radius:12px;padding:18px;vertical-align:top;width:32%;">
          <p style="font-size:26px;margin:0 0 8px;text-align:center;">📖</p>
          <p style="font-size:12px;font-weight:800;color:#f1f5f9;margin:0 0 6px;text-align:center;">${t.step2title}</p>
          <p style="font-size:11px;color:#94a3b8;margin:0;text-align:center;line-height:1.5;">${t.step2desc}</p>
        </td>
        <td style="width:2%;"></td>
        <td style="background:rgba(74,222,128,0.06);border:1px solid rgba(74,222,128,0.15);border-radius:12px;padding:18px;vertical-align:top;width:32%;">
          <p style="font-size:26px;margin:0 0 8px;text-align:center;">🎬</p>
          <p style="font-size:12px;font-weight:800;color:#f1f5f9;margin:0 0 6px;text-align:center;">${t.step3title}</p>
          <p style="font-size:11px;color:#94a3b8;margin:0;text-align:center;line-height:1.5;">${t.step3desc}</p>
        </td>
      </tr>
    </table>
    ...
    <!-- PLATEFORMES -->
    <div style="text-align:center;margin-bottom:24px;">
      <p style="font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#475569;margin:0 0 16px;">${t.platforms}</p>
      <table cellpadding="0" cellspacing="0" style="margin:0 auto;">
        <tr>
          ${platforms.map(p => `
          <td style="padding:0 4px;text-align:center;">
            <div style="background:${p.color}15;border:1px solid ${p.color}30;border-radius:8px;padding:7px 10px;">
    ...
    <!-- ARTICLES -->
    <p style="font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#94a3b8;margin:0 0 16px;">${t.articlesLabel}</p>
    ${articles.map(a => `
    <a href="${a.url}" style="text-decoration:none;display:block;margin-bottom:10px;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:12px;overflow:hidden;">
        <tr>
          <td style="background:${a.color};width:4px;padding:0;"></td>
          <td style="padding:14px 16px;">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td>
                  <p style="font-size:14px;font-weight:700;color:#f1f5f9;margin:0 0 4px;line-height:1.3;">${a.emoji} ${a.title}</p>
                  <p style="font-size:11px;color:#64748b;margin:0;font-weight:600;">${a.label} · ${a.time} ${t.readTime}</p>
                </td>
    ...
    <!-- CTA -->
    <div style="text-align:center;">
      <p style="font-size:14px;font-weight:700;color:#f1f5f9;margin:0 0 6px;">${t.ctaTitle}</p>
      <p style="font-size:13px;color:#64748b;margin:0 0 16px;">${t.ctaSub}</p>
      <img src="${APP_URL}/${isFr ? 'cliffhangers%20.png' : 'cliffhangers%20EN.png'}" alt="Exemples" width="516" style="width:100%;max-width:516px;display:block;margin:0 auto 20px;border-radius:12px;" />
