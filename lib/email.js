...
function relanceHtml() {
  ...
  return base("Ta première série t'attend", `
    ...
    <p style="font-size:15px;color:#6B6B68;line-height:1.7;margin:0 0 24px;text-align:center;">
      Tu t'es abonné(e) il y a 3 jours. En 30 secondes, tu peux avoir une bible complète, un séquencier et des scripts prêts à tourner.
    </p>
    <div style="background:#0F1A12;border-radius:14px;padding:20px;margin-bottom:24px;">
      ...
      ${packs.map(p => `
        <div style="display:flex;align-items:center;gap:12px;padding:12px;background:#1a2a1e;border-radius:10px;margin-bottom:8px;">
          <span style="font-size:24px;">${p.emoji}</span>
          <div>
            <p style="font-size:14px;font-weight:700;color:#fff;margin:0 0 2px;">${p.label}</p>
            <p style="font-size:12px;color:#6a7a6e;margin:0;">${p.desc}</p>
          </div>
        </div>
      `).join("")}
    </div>
